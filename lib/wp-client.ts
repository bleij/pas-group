...
const DEV_TTL = Number(process.env.WP_DEV_CACHE_TTL ?? 120); // сек, можно 300
...
const MAX_CONCURRENT = Number(process.env.WP_MAX_CONCURRENT ?? 1); // сделаем 1 в .env.local
let active = 0;
    ...
    const p = schedule<T>(async () => {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
            const res = await fetch(endpoint, {
                ...
                body,
                // ВАЖНО: только ISR через revalidate, БЕЗ cache:'no-store'
                next: {revalidate},
            });

            if (res.status === 429) {
                const retryAfter = Number(res.headers.get("retry-after") || 0);
                const jitter = Math.random() * 300; // чуть-чуть рассинхрона
                const delay = retryAfter > 0
                    ? retryAfter * 1000
                    : Math.min(2000 * 2 ** attempt, 15000) + jitter;
            ...
            if (!ct.includes("application/json")) {
                const text = await res.text();
                throw new Error(`WP non-JSON (${res.status}): ${text.slice(0, 200)}`);
            }

            const json = await res.json();
            if (json?.errors) {
                throw new Error(`WP GraphQL errors: ${JSON.stringify(json.errors).slice(0, 500)}`);
            }

            const data = json.data as T;
            setCache(key, data); // сохранить в dev-кэш
            return data;
        }

        throw new Error("WP: exhausted retries after 429/backoff");
    });
