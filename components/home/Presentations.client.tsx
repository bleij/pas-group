...
export default function PresentationsClient({
                                                items,
                                                showMoreHref,
                                                bgColor = "bg-gray-100", // <-- добавили дефолтный фон
                                            }: {
    items: Presentation[];
    showMoreHref?: string;
    bgColor?: string; // <-- новый проп
}) {
    return (
        <section id="presentations" className={`w-full ${bgColor} py-4 sm:py-8 md:py-14`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
                    {items.map((p) => (
                        ...
                        >
                            ...
                            <div
                                className="hidden md:flex absolute inset-0 bg-black/40 items-center justify-center text-white text-sm opacity-0 group-hover:opacity-100 transition">
                                Открыть
                            </div>
                        </button>
                    ))}
                ...
                {showMoreHref && (
                    <div className="mt-6 sm:mt-8 md:mt-10 flex justify-start">
                        <a
                            href={showMoreHref}
                            className="inline-block px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 md:py-3 bg-gray-200 rounded-md hover:bg-gray-300 transition font-medium text-sm md:text-base text-[#374151]"
                        >
                            Все презентации →
                        </a>
                    </div>
